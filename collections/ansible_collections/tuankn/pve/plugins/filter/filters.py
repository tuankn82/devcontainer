# (c) 2024 tuankn <daitintdk@gmail.com>
                ...
                if match := re.match(r"([0-9a-f]+) [ *]" + filename, line):
                    ...
                    if length == 64:
                        ...
                        raise AnsibleFilterError(
                            f"Unexpected checksumn length {
                                length}./ Expected 64 or 128."
                        )
    except IOError as e:
        raise AnsibleFilterError(
...
def qcow2_image_name(filename: str) -> str:
    if filename.endswith(".img"):
        filename = filename.replace(".img", ".qcow2")
    elif not filename.endswith(".qcow2"):
        filename += ".qcow2"
    return filename

# def replace_img_with_qcow2(filename: str) -> str:
#     if filename.endswith(".img"):
